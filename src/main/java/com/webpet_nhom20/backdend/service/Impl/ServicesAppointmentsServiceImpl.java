...
public class ServicesAppointmentsServiceImpl implements ServicesAppointmentsService {
        ...
        public ServiceAppointmentsResponse createByEmail(
                ...
                BookingTime bookingTime = bookingTimeRepository.findByIdForUpdate(
                                request.getBookingTimeId())
                                .orElseThrow(() -> new RuntimeException("BookingTime không tồn tại"));

                ServicesPet service = bookingTime.getService();

                // 2️⃣ Check rule: phải đặt trước cutoff
                LocalDateTime slotStart = bookingTime.getSlotDate()
                                .atTime(bookingTime.getStartTime());

                int bookingCutoffMinutes = 120;

                LocalDateTime cutoffTime = slotStart.minusMinutes(bookingCutoffMinutes);
                ...
                LocalDateTime end = bookingTime.getSlotDate()
        ...
        public ServiceAppointmentsResponse create(
                ...
                ServiceAppointments appointment = mapToEntity(request, bookingTime, user);
        ...
        public ServiceAppointmentsResponse update(
                ...
                ServiceAppointments appointment = servicesAppointmentsRepository.findById(request.getId())
                                .orElseThrow(() -> new AppException(ErrorCode.APPOINTMENT_NOT_FOUND));

                LocalDateTime now = LocalDateTime.now();
                LocalDateTime updateDeadline = appointment.getAppointmentStart().minusMinutes(120);
                ...
                if (request.getBookingTimeId() != null &&
                        ...
                        // 1️⃣ LOCK slot cũ
                        BookingTime oldSlot = bookingTimeRepository.findByIdForUpdate(
                                        appointment.getBookingTime().getId())
                                        .orElseThrow();

                        // 2️⃣ LOCK slot mới
                        BookingTime newSlot = bookingTimeRepository.findByIdForUpdate(
                                        request.getBookingTimeId())
                                        .orElseThrow(() -> new AppException(ErrorCode.BOOKING_TIME_NOT_FOUND));

                        ServicesPet service = newSlot.getService();

                        // 3️⃣ Validate giờ đặt (giống create)
                        LocalDateTime slotStart = newSlot.getSlotDate().atTime(newSlot.getStartTime());

                        LocalDateTime latestBookingTime = slotStart.minusMinutes(service.getDurationMinutes());
        ...
        @Override
        public Page<ServiceAppointmentsResponse> getAppointmentsWithFilter(
                        FilterAppointmentRequest request,
                        Pageable pageable) {
                // Parse status
                AppoinmentStatus status = null;
                if (request.getStatus() != null && !request.getStatus().isEmpty()) {
                        try {
                                status = AppoinmentStatus.valueOf(request.getStatus());
                ...
                if (request.getFromDate() != null && !request.getFromDate().isEmpty()) {
                        LocalDate from = LocalDate.parse(request.getFromDate(), dateFormatter);
                        fromDate = from.atStartOfDay();
                }

                if (request.getToDate() != null && !request.getToDate().isEmpty()) {
                        LocalDate to = LocalDate.parse(request.getToDate(), dateFormatter);
                        toDate = to.atTime(23, 59, 59);
                }

                Page<ServiceAppointments> page = servicesAppointmentsRepository.findWithFilters(
                                status, email, serviceId, fromDate, toDate, pageable);
        ...
        public AppointmentStatisticsResponse getStatistics() {
                ...
                long todayCount = servicesAppointmentsRepository.countTodayAppointments(today);
                long scheduledCount = servicesAppointmentsRepository.countByStatus(AppoinmentStatus.SCHEDULED);
                long completedCount = servicesAppointmentsRepository.countByStatus(AppoinmentStatus.COMPLETED);
                long totalCount = servicesAppointmentsRepository.countAllAppointments();
        ...
        private ServiceAppointmentsResponse mapToResponse(
                ...
                ServiceAppointmentsResponse r = new ServiceAppointmentsResponse();
