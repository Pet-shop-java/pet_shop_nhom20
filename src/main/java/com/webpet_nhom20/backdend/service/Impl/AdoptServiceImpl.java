...
public class AdoptServiceImpl implements AdoptService {
        ...
        public Page<AdoptResponse> getAdoptsByUser(int userId, String code, String status, String isDeleted,
                        Pageable pageable) {
                Page<Adopt> page = adoptRepository.findAdoptsByUserWithFilters(userId, code, status, isDeleted,
                                pageable);
        ...
        public Page<AdoptResponse> getAllAdopts(Integer petId, String code, String status, String isDeleted,
                        Pageable pageable) {
        ...
        public AdoptResponse updateStatusAdopt(int adoptId, String status) {
                ...
                // Gửi mail khi COMPLETED hoặc REJECTED
                if (AdoptStatus.COMPLETED.name().equals(status)
                                || AdoptStatus.REJECTED.name().equals(status)) {

                        // Chỉ COMPLETED mới xử lý pet
                        if (AdoptStatus.COMPLETED.name().equals(status)) {
                        ...
                        int count = adoptRepository
                                        .countByPetIdAndStatusAndIsDeleted(
                                                        adopt.getPetId(),
                                                        "PENDING",
                                                        "0");
                        if(count == 0){
                        ...
                        // 🔹 LẤY DỮ LIỆU ADOPT (KHÔNG CÓ EMAIL)
                        AdoptDetailProjection detail = adoptRepository.findAdoptDetail(adoptId)
                                        .orElseThrow(() -> new RuntimeException(
                                                        "Không tìm thấy AdoptDetail để gửi email"));

                        // 🔹 LẤY EMAIL THEO USER ID
                        String email = userRepository.findEmailByUserId(detail.getUserId())
                                        .orElseThrow(() -> new RuntimeException("Không tìm thấy email của user"));

                        // 🔹 BUILD & SEND MAIL
                        String subject = CommonUtil.buildAdoptEmailSubject(
                                        detail.getStatus(),
                                        detail.getCode());

                        String html = CommonUtil.buildAdoptEmailHtml(detail);

                        // DEBUG: Log all email info
                        System.out.println("=== DEBUG ADOPT EMAIL ===");
                        System.out.println(">>> Email to: " + email);
                        System.out.println(">>> Subject: " + subject);
                        System.out.println(">>> Status: " + detail.getStatus());
                        System.out.println(">>> Code: " + detail.getCode());
                        System.out.println(">>> HTML length: " + (html != null ? html.length() : 0));
                        System.out.println("=========================");
