...
public interface StatisticsRepository extends JpaRepository<Order, Integer> {
        ...
        /**
         * Get revenue by day within a date range
        ...
        /**
         * Get revenue by week within a date range
         * Returns: [year, week_number, revenue, order_count]
         */
        @Query(value = """
                        SELECT
                            YEAR(created_date) as year,
                            WEEK(created_date, 1) as week_number,
                            COALESCE(SUM(final_amount), 0) as revenue,
                            COUNT(*) as order_count
                        FROM orders
                        WHERE status IN ('COMPLETED', 'DELIVERED')
                        AND is_deleted = '0'
                        AND DATE(created_date) BETWEEN :startDate AND :endDate
                        GROUP BY YEAR(created_date), WEEK(created_date, 1)
                        ORDER BY year ASC, week_number ASC
                        """, nativeQuery = true)
        List<Object[]> getWeeklyRevenue(@Param("startDate") String startDate, @Param("endDate") String endDate);
}
